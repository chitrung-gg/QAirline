...
export default function Page(props: { params: { id: string } }) {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const res = await axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_URL}:${process.env.NEXT_PUBLIC_BACKEND_PORT}/news/${id}`, formData);
            onOpen();
    ...
    const handleDelete = async () => {
        ...
        // if (confirmDelete) {
            try {
                await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}:${process.env.NEXT_PUBLIC_BACKEND_PORT}/news/${id}`);
                onDeletedOpen();
            } catch (error) {
                console.error(error);
                onErrorOpen();
